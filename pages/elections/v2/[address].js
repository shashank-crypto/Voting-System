...
function ElectionInteract({ query }) {
    ...
    useEffect(() => {
        async function getInfo() {
            ...
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    const user = (await getDoc(doc(db, "users", user.uid))).data();
                    setCurrUser(user);
                }
            })
    ...
    async function registerAsVoter() {
        if (currUser.email) {
            await registerVoter(currUser.name, currUser.age, currUser.aadhar, currUser.phone, info[3]);
        }
    }

    async function registerAsCandidate() {
        if (currUser.email) {
            await registerCandidate(currUser.name, currUser.party, currUser.age, currUser.aadhar, info[3]);
        }
    }
